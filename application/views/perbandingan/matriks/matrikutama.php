...
function rk()
{
	...
	for(i=1;i<=<?=$jumlah;?>;i++)
	{
		...
		var priork = $("#priork-b"+i).val();
		var jmlrk = $("#jmlrk-b"+i).val();
		var hasilrk = parseFloat(priork)*parseFloat(jmlrk);
		var hasilhitung = hasilrk;
		total += hasilhitung;
		$("#hasilrk-b"+i).val(hasilhitung);
	}
	var fx2=total;
	$("#totalrk").val(fx2);
	$("#sumrk").val(fx2);
	...
	var ci=parseFloat($("#sumrk").val() - <?= $jumlah?>)/(parseFloat(<?=$jumlah;?>) - 1);
	var fx_ci=ci;
	$("#sumci").val(fx_ci);
